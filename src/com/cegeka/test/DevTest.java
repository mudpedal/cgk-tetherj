...
public class DevTest {
	...
	public static void main(String[] args) {
		...
		String mortal = "contract mortal { address owner; function mortal() { owner = msg.sender; } function kill() { if (msg.sender == owner) suicide(owner); } }";
		String sourceCode = "contract mortal { address owner; function mortal() { owner = msg.sender; } function kill() { if (msg.sender == owner) suicide(owner); } } contract greeter is mortal { string greeting; uint256 p; function greeter(string _greeting, uint256 _p) public { greeting = _greeting; p=_p; } function setGreeting(string _greeting, uint256 _p) public returns(string, uint256)  { greeting = _greeting; p=_p; return (greeting, p);} function greet() constant returns (string, uint256) { return (greeting,p); } }";
		...
		BlocklinksResponse<Contracts> contractResponse = service.compileSolidity(sourceCode);
		...
		Contracts contracts = contractResponse.getValue();
					...
					if (response.getErrorType() != null) {
						...
						try {
							...
							if (newGreeting.length > 0) {
								...
								EthTransaction tx = contract.callModMethod(wallet, "setGreeting",
										"AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA",
										BigInteger.valueOf(69));
								tx.setGasLimit(EthTransaction.maximumGasLimit);
