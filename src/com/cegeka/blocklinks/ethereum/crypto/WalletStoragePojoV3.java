...
public class WalletStoragePojoV3 {
	...
	public static WalletStoragePojoV3 createWallet(String passphrase) {
		...
		String [] ecdsa = Util.generateECDSAPair();
		
		String privateKey = ecdsa[0];
		wallet.address = ecdsa[1];
		...
		try {
			...
			// what to crypt
			byte[] privKeyBytes = Util.hexToBytes(privateKey);

			// key will only be the first 16 bytes of the hash key
			byte[] trimmedKey = Arrays.copyOfRange(key, 0, 16);
			
			// macKey that will be used to validate wallet unlocking (as per ethereum standard)
