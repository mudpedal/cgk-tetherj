...
public class DevTest {
    ...
    public static void main(String[] args) {
        ...
        try {
            sourceCode = String.join("\n", Files.readAllLines(Paths.get("/home/andreicg/untouched/DAO/FULL.sol")));
        ...
        if (contractResponse.getErrorType() != null) {
            System.out.println("FAILED TO COMPILED CONTRACT, reason: "
                    + contractResponse.getException().getMessage() + " "
                    );
            return;
        }
        ...
        System.out.println("Compiled " + contractNames.length + " contracts: "
                + Arrays.toString(contractNames));

        /*
        System.out.println("Choosing second contract named " + contractNames[1]);
        ContractData firstContract = contractResponse.getValue()
                .getContractByName(contractNames[1]);
        ...
        try {
            ...
            EthTransaction tx = newFactory.createContract("Hello World!",
            ...
            service.listenForTxReceipt(txHash, new TetherjHandle<TransactionReceipt>() {
                ...
                public void call(TetherjResponse<TransactionReceipt> response) {
                    if (response.getErrorType() != null) {
                        ...
                        List<String> logs = service.getRpcClient()
                                .getPendingTransactionFilterChanges(filterId);
                        System.out.println(Arrays.toString(logs.toArray()));
                        System.out.println("Tx mined, receipt: " + response.getValue().toString());
                        EthSmartContract contract = factory
                                .getContract(response.getValue().getContractAddress());
                        ...
                        try {
                            ...
                            TetherjResponse<List<Object[]>> greetingsResponse = service
                                    .makeCalls(calls);
                            List<Object[]> greetings = greetingsResponse.getValue();
                            System.out.println(
                                    "Greetings are " + Arrays.deepToString(greetings.toArray()));

                            EthCall call = contract.dryCallModMethod(
                                    wallet.getStorage().getAddress(), "setGreeting",
                            ...
                            if (newGreeting.length > 0) {
                                ...
                                System.out.println(CryptoUtil.byteToHex(encoded));
                                TetherjResponse<String> txHashResponse = service
                                        .sendTransaction(wallet, tx);
                                System.out.println(
                                        "Sending transaction " + txHashResponse.getValue());
                                service.listenForTxReceipt(txHashResponse.getValue(),
                                    ...
                                    public void call(TetherjResponse<TransactionReceipt> response) {
                                        if (response.getErrorType() != null) {
                                            ...
                                            System.out.println(Arrays.toString(logs.toArray()));
                                            System.out
                                                    .println("Tx mined for set greeting, receipt: "
                                                            + response.getValue().toString());
                                            Object[] greeting;
                                            try {
                                                EthCall call = contract.callConstantMethod("greet");
                                                TetherjResponse<Object[]> greetingResponse = service
                                                        .makeCall(call);
                                                greeting = greetingResponse.getValue();

                                                System.out.println(
                                                        "Greeting is " + greeting[0].toString());
