 ...
 *
 * <p>
 * A key can be <i>compressed</i> or <i>uncompressed</i>. This refers to whether the public key is
 * represented when encoded into bytes as an (x, y) coordinate on the elliptic curve, or whether
...
public class ECKey implements Serializable {
    ...
    public static ECKey recoverFromSignature(int recId, ECDSASignature sig, byte[] messageHash,
        ...
        BigInteger prime = curve.getQ(); // Bouncy Castle is not consistent about the letter it uses
                                         // for the prime.
    ...
    public int hashCode() {
        ...
        byte[] bits = getPubKey();
        return (bits[0] & 0xFF) | ((bits[1] & 0xFF) << 8) | ((bits[2] & 0xFF) << 16)
                | ((bits[3] & 0xFF) << 24);
    }
