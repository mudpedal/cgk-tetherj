 ...
 * sent by an actor external to Ethereum. An external actor can be a person (via
 * a mobile device or desktop computer) or could be from a piece of automated
 * software running on a server. There are two types of transactions: those
 * which result in message calls and those which result in the creation of new
 * contracts.
 */
public class Transaction {
	...
	/*
	 * the address of the destination account In creation transaction the
	 * receive address is - 0
	 */
	private byte[] receiveAddress;

	/*
	 * the amount of ether to pay as a transaction fee to the miner for each
	 * unit of gas
	 */
	private byte[] gasPrice;

	/*
	 * the amount of "gas" to allow for the computation. Gas is the fuel of the
	 * computational engine; every computational step taken and every byte added
	 * to the state or transaction list consumes some gas.
	 */
	private byte[] gasLimit;

	/*
	 * An unlimited size byte array specifying input [data] of the message call
	 * or Initialization code for a new contract
	 */
	private byte[] data;
	...
	/*
	 * creation contract tx [ nonce, gasPrice, gasLimit, "", endowment, init,
	 * signature(v, r, s) ] or simple send tx [ nonce, gasPrice, gasLimit,
	 * receiveAddress, value, data, signature(v, r, s) ]
	 */
	...
	public long transactionCost() {
		...
		long zeroVals = data.length - nonZeroes;
	...
	public String toString() {
		...
		return "TransactionData [" + "hash=" + ByteUtil.toHexString(hash) + "  nonce=" + ByteUtil.toHexString(nonce)
				...
				+ ", receiveAddress=" + ByteUtil.toHexString(receiveAddress) + ", value=" + ByteUtil.toHexString(value)
				+ ", data=" + ByteUtil.toHexString(data) + ", signatureV=" + (signature == null ? "" : signature.v)
				+ ", signatureR="
